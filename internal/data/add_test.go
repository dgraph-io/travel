...
// TestAdd validates all the adding support in data.
...
func addCity(t *testing.T) {
	...
	t.Log("Given the need to be able to validate storing a city.")
	{
		...
		t.Logf("\tTest %d:\tWhen handling a city for Sydney.", testID)
		{
			...
			cityAdd.ID = ""
			cityAdd, err = db.Add.City(ctx, cityAdd)
			if err == nil {
	...
	t.Log("Given the need to be able to validate storing a place.")
	{
		...
		t.Logf("\tTest %d:\tWhen handling a place for sydney.", testID)
		{
			...
			for i, place := range places {
				addPlace, err := db.Add.Place(ctx, cityAdd.ID, place)
				if err != nil {
...
// 			addAdvisory, err := db.Add.Advisory(ctx, cityID, addAdvisory)
// 			if err != nil {
...
// 			addWeather, err := db.Add.Weather(ctx, cityID, addWeather)
// 			if err != nil {
