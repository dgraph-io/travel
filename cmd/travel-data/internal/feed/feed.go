...
// Work2 retrieves and stores the feed data for this API.
func Work2(log *log.Logger, search Search, keys Keys, dbHost string) error {
	ctx := context.Background()
	...
	log.Printf("feed : Work : Search : cityID[%s] city[%+v] filter[%+v]", cityID, city, filter)
	...
	for i := 0; i < 1; i++ {
		...
		if errRet != nil && errRet != io.EOF {
			log.Print("feed : Work : Search : ERROR : ", errRet)
			return ErrFailed
		}
		log.Printf("feed : Work : Search : Result\n%+v", places)
		...
		for _, place := range places {
			if err := data.StorePlace(ctx, log, cityID, place); err != nil {
				log.Printf("feed : Work : Store : ERROR : %s : %+v", err, place)
				continue
			}
			log.Printf("feed : Work : Store : Success : %+v", place)
		}
